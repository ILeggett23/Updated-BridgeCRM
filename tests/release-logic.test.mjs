...
test("release metadata advances to 1.3.28", () => {
  assert.equal(release.APP_RELEASE.version, "1.3.28");
...
test("continuing records the exact release and suppresses the same version", () => {
  ...
  assert.equal(storage.getItem(release.RELEASE_STORAGE_KEY), "1.3.28");
...
test("a future version is not suppressed by the current viewed value", () => {
  const storage = memoryStorage({ bridgeLastSeenVersion: "1.3.28" });
...
test("service worker uses the semantic release cache", () => {
  assert.match(serviceWorker, /bridge-app-v1\.3\.28/);
  assert.match(serviceWorker, /config\.js\?v=1\.3\.28/);
});
