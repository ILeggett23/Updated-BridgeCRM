...
test("production serves scripts as JavaScript without corrupting selector helpers", () => {
  ...
  assert.ok(page.includes('<script type="module" src="./app.js?v=1.3.19"'));
...
test("v1.3.19 cache busting is coordinated across scripts, styles, manifest, and service worker", () => {
  assert.ok(page.includes("./config.js?v=1.3.19"));
  assert.ok(page.includes("./styles.css?v=1.3.19"));
  assert.ok(page.includes("./engagement-logic.js?v=1.3.19"));
  assert.ok(page.includes("./release-logic.js?v=1.3.19"));
  assert.ok(page.includes("./app.js?v=1.3.19"));
  assert.ok(worker.includes("bridge-app-v1.3.19"));
});
...
test("the GitHub Pages client opens locally without loading the Cloudflare account gate", async () => {
  ...
  assert.ok(serviceWorker.includes('importScripts(new URL("config.js?v=1.3.19", ROOT).href)'));
