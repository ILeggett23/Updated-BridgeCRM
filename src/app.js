...
function quickCaptureConversationForm(mode,contacts) {
  ...
  const learned=`${field(meeting?"What did you discuss?":"What did you learn?",`<textarea name="notes" required data-guide-target="capture-notes" placeholder="${meeting?"Key points from the meeting":"A useful detail, need, goal, or next step"}"></textarea>`)}${field("Add to What I Know",'<textarea name="personalInfo" data-guide-target="capture-context" placeholder="Optional relationship context"></textarea>')}`;
  const next=`${quickCaptureNextAction()}<div data-guide-target="capture-tracking">${quickCaptureAdvanced(`${field("Date and time",`<input name="conversationDate" type="datetime-local" value="${dateTimeLocalValue(new Date())}" required>`)}${quickCaptureTrackingFields()}`,"Date, stage, and activity details")}</div>`;
...
function quickCreateModal() {
  ...
  let content=`<div class="quick-create-list capture-action-grid" data-guide-target="capture-types">${choices.map(([mode,title,description,iconName])=>`<button class="quick-create-option capture-action" type="button" data-quick-mode="${mode}" ${mode==="conversation"?'data-guide-target="capture-conversation"':""}><span class="quick-create-icon">${icons[iconName]}</span><span><strong>${title}</strong><small>${description}</small></span></button>`).join("")}</div>`;
...
function renderDashboard() {
  ...
  return `<section class="today-home" aria-label="Today">
    ...
    <section class="today-home__section today-home__next" aria-labelledby="today-next-up" data-guide-target="today-next-up"><h2 id="today-next-up">Next up</h2>${next ? todayNextAction(next, now) : todayClearAction()}</section>
...
function renderContacts() {
  ...
  return `<section class="people-home" aria-label="People" >
    <header class="people-home__header"><h1 class="primary-page-title">People</h1><button data-guide-target="people-places" class="people-home__places" type="button" data-people-contact-mode="places" aria-label="Browse places">${icons.location}</button></header>
    ${SearchField({ id:"contactSearch", value:ui.search, placeholder:"Search people", label:"Search people", className:"people-home__search", attributes:'data-guide-target="people-search"' })}
    <div data-guide-target="people-filters" class="people-home__filters" role="group" aria-label="Quick people filters">${["All","Recent","Priority","Prospects","Customers"].map(label => Chip(label, { active:ui.peopleQuick===label, className:"people-home__quick-filter", attributes:`data-people-quick="${label}"` })).join("")}${peopleFiltersTrigger()}</div>
...
function renderPipeline(connectionState="") {
  ...
  const tabs=Tabs([{label:"Prospect",value:"Prospect",active:ui.pipelineRole==="Prospect",attributes:'data-pipeline-role="Prospect"'},{label:"Customer",value:"Customer",active:ui.pipelineRole==="Customer",attributes:'data-pipeline-role="Customer" data-guide-target="customer-tab"'}],{label:"Pipeline type",className:"pipeline-home__tabs",idPrefix:"pipeline-role"});
  return `<section class="contacts-route pipeline-home" aria-label="Pipeline" ><div data-guide-target="pipeline-tabs"><header class="pipeline-home__header"><h1 class="primary-page-title">Pipeline</h1></header>${tabs}</div>${connectionState}<div id="pipeline-role-panel-${escapeHTML(ui.pipelineRole)}" role="tabpanel" aria-labelledby="pipeline-role-tab-${escapeHTML(ui.pipelineRole)}">${ui.pipelineRole==="Prospect"?renderProspectPipeline(prospectContacts,now):renderCustomerPipeline(customerContacts,now)}</div>${ui.pipelineStageDetail?prospectStageDetailSheet(ui.pipelineStageDetail,prospectContacts,now):""}${selectedContact?prospectTransitionSheet(selectedContact):""}${ui.customerPipelineStageDetail?customerStageDetailSheet(ui.customerPipelineStageDetail,customerContacts,now):""}${selectedCustomerContact?customerTransitionSheet(selectedCustomerContact):""}</section>`;
}
...
function insightsPipelineIntelligence(model) {
  ...
  return `<section data-guide-target="insights-pipeline" class="insights-section insights-intelligence" aria-labelledby="insights-intelligence-title"><h2 id="insights-intelligence-title">Pipeline intelligence</h2>${intelligence||activity?`<div class="insights-intelligence__list">${intelligence}${activity}</div>`:emptyInline("No pipeline signals yet","Stalled relationships and verified follow-up or stage activity will appear here.")}</section>`;
}
...
function analyticsDetailActivity(model) {
  ...
  return `<section class="analytics-detail-section analytics-detail-activity"><h3>Activity</h3>${model.conversations.length?`<div class="analytics-detail-chart-scroll"><div class="analytics-detail-chart" style="--insights-points:${model.daySeries.length};--analytics-chart-min:${minimumWidth}px" role="img" aria-label="Conversation activity: ${escapeHTML(model.daySeries.map(point=>`${point.date} ${point.value}`).join(", "))}">${model.daySeries.map(point=>`<div><span class="analytics-detail-chart__column"><span>${point.value||""}</span><i class="${point.value?"has-value":""}" style="height:${point.value?Math.max(6,Math.round(point.value/max*70)):3}px"></i></span><small>${escapeHTML(point.label)}</small></div>`).join("")}</div></div>`:emptyInline("No conversations in this period","Counted conversations will fill this chart without inventing activity.")}</section>`;
}
function insightsDetailedAnalytics(model, scorecard, previousScorecard, { embedded=true }={}) {
  ...
  return `<details data-guide-target="analytics-periods" class="insights-details"><summary><span><strong>Detailed analytics</strong><small>Goals, outcomes, trends, and stage movement</small></span>${icons.chevronRight}</summary>${body}</details>`;
}
function renderAnalytics() {
  ...
  return `<section class="analytics-workspace insights-home" aria-label="Insights" ><div data-guide-target="insights-week">${pageHead("Insights", "Relationship activity and momentum from your existing Bridge data.", IconButton("share","Share scorecard",{attributes:'id="shareScorecard"'}), "primary-page-title")}</div><section class="insights-hero" aria-labelledby="insights-period-summary"><span>${escapeHTML(analyticsPeriodEyebrow())}</span>${periodActivity?`<h2 id="insights-period-summary">${summary}</h2><p>${followUpSummary}${placeSummary}</p>`:`<div id="insights-period-summary">${emptyInline("No activity in this period","Open Detailed analytics to choose another period, or log a conversation to begin your Insights history.")}</div>`}</section>${insightsConversationChart(model)}${insightsPipelineIntelligence(model)}<section data-guide-target="insights-distribution" class="insights-section insights-pipeline-snapshot" aria-labelledby="insights-pipeline-snapshot"><h2 id="insights-pipeline-snapshot">Pipeline activity</h2><p class="insights-section__description">Current stage distribution uses Bridge’s exact Prospect and Customer stages.</p>${insightsStageSnapshot("Prospect",model)}${insightsStageSnapshot("Customer",model)}</section>${insightsFollowUpEffectiveness(model)}${insightsPlaces(model)}${insightsDetailedAnalytics(model,scorecard,previousScorecard)}</section>`;
}
...
function settingsNavigationRow(title,detail,section,iconName="gear",meta=""){
  return `<button class="settings-nav-row" type="button" data-settings-section-open="${escapeHTML(section)}" data-guide-target="settings-${escapeHTML(section)}"><span class="settings-nav-row__copy"><strong>${escapeHTML(title)}</strong>${detail?`<small>${escapeHTML(detail)}</small>`:""}</span><span class="settings-nav-row__chevron" aria-hidden="true">${icons.chevronRight}</span></button>`;
}
...
function settingsRootContent(s,metrics){
  ...
  const groups=[
    ...
    settingsNavigationGroup("Help · Bridge Guide",settingsActionRow("Replay Bridge Guide","Practice the complete relationship workflow again",'data-replay-walkthrough data-guide-target="guide-replay"')),
...
async function scorecardPreviewPNG(scorecard, { format = "preview" } = {}) {
  ...
  if (!context) return "";
  const brandMark = await scorecardBrandMark();
...
async function scorecardImageFile(scorecard) {
  const base64 = await scorecardPreviewPNG(scorecard, { format: "image" });
...
async function shareScorecardImage() {
  ...
  const file = await scorecardImageFile(scorecard);
...
async function createScorecardLink() {
  ...
  const previewPNG = await scorecardPreviewPNG(snapshot);
  const options = {
    method: "POST",
