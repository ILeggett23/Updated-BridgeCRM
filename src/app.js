...
function render() {
  ...
  app.innerHTML = `<div class="app-shell bridge-pattern-shell" ${ui.accountAction?'aria-hidden="true" inert':''}>
    ...
    <nav class="nav bridge-pattern-nav" aria-label="Primary navigation">
      ${navButton("dashboard", "Today", "home")}
      ${peopleNavButton()}
      ${quickCreateButton()}
      ${pipelineNavButton()}
      ${navButton("analytics", "Insights", "chart")}
      <span class="nav-selection-indicator" aria-hidden="true" style="--nav-selection-index:${Math.max(0,nextNavSelection)};--nav-selection-visible:${nextNavSelection<0?0:1}"></span>
    </nav>
  </div>${ui.settingsOpen && ui.routedScreen!=="settings" ? settingsModal() : ""}${ui.achievementsOpen && ui.routedScreen!=="achievements" ? achievementsModal() : ""}${ui.quickCreateOpen ? quickCreateModal() : ""}${ui.peopleFiltersOpen ? peopleFilterSheet(peopleVisibleContacts().length) : ""}${ui.placeDetailId && ui.routedScreen!=="place" ? placeDetailSheet(ui.placeDetailId) : ""}${ui.detailId && !["person","person-edit"].includes(ui.routedScreen) ? contactModal(ui.detailId) : ""}${ui.activityHistoryContactId && ui.routedScreen!=="person-timeline" ? activityHistoryModal(ui.activityHistoryContactId) : ""}${ui.communicationContactId ? communicationLogModal(ui.communicationContactId) : ""}${ui.scorecardShareOpen && ui.routedScreen!=="scorecard" ? scorecardShareModal() : ""}${ui.releaseNotesOpen ? releaseNotesModal() : ""}${ui.accountMigrationOpen ? accountMigrationModal() : ""}${ui.accountAction ? accountActionModal() : ""}`;
...
function peopleNavButton() { const active=ui.page==="contacts"&&ui.contactMode!=="pipeline"; return `<button type="button" class="nav-button ${active ? "active" : ""}" data-open-people aria-label="People" ${active ? 'aria-current="page"' : ""}>${icons.people}<span>People</span></button>`; }
function pipelineNavButton() { const active=ui.page==="contacts"&&ui.contactMode==="pipeline"; return `<button type="button" class="nav-button ${active ? "active" : ""}" data-open-pipeline aria-label="Pipeline" ${active ? 'aria-current="page"' : ""}>${icons.network}<span>Pipeline</span></button>`; }
function quickCreateButton() { const active=ui.quickCreateOpen||ui.page==="add"; return `<button type="button" class="nav-button quick-create-button ${active ? "active" : ""}" id="quickCreateButton" aria-label="Capture what happened" aria-haspopup="dialog" aria-expanded="${ui.quickCreateOpen}" ${active ? 'aria-current="page"' : ""}>${icons.plus}<span>Capture</span></button>`; }
function navSelectionIndex() { if(ui.quickCreateOpen||ui.page==="add")return -1;if(ui.page==="dashboard")return 0;if(ui.page==="contacts")return ui.contactMode==="pipeline"?3:1;if(ui.page==="analytics")return 4;return -1; }
...
function PresentationScreen(content, { className="", title="", eyebrow="", action="", back=true, large=false }={}) {
  return `<section class="presentation-screen presentation-screen--${escapeHTML(ui.routeDirection)}${className?` ${className}`:""}" data-presentation-screen="${escapeHTML(ui.routedScreen||"")}">${ScreenHeader(title,{eyebrow,action,back,large})}<div class="presentation-screen__body">${content}</div></section>`;
}

// Shared Bridge design-system primitives. Feature routes consume these while
// keeping their existing data contracts, IDs, and workflow event bindings.
function Button(content, { tone="secondary", size="medium", className="", attributes="", type="button" }={}) { const tones=["primary","secondary","quiet","danger"]; const sizes=["small","medium","large"]; return `<button type="${type}" class="ui-button ui-button--${tones.includes(tone)?tone:"secondary"} ui-button--${sizes.includes(size)?size:"medium"}${className?` ${className}`:""}" ${attributes}>${content}</button>`; }
...
function Tabs(items, { label="Sections", className="", idPrefix="" }={}) { const prefix=idPrefix||`ui-tabs-${++sharedPrimitiveId}`; return `<div class="ui-tabs${className?` ${className}`:""}" role="tablist" aria-label="${escapeHTML(label)}">${items.map(({label,value,active=false,attributes=""})=>`<button type="button" id="${escapeHTML(prefix)}-tab-${escapeHTML(value)}" role="tab" data-value="${escapeHTML(value)}" aria-controls="${escapeHTML(prefix)}-panel-${escapeHTML(value)}" aria-selected="${active}" tabindex="${active?"0":"-1"}" ${attributes}>${escapeHTML(label)}</button>`).join("")}</div>`; }
...
function quickCaptureTrackingFields() { return `${field("Move to stage",`<select name="pipelineStage" data-capture-stage>${quickCaptureStageOptions()}</select>`)}<div class="quick-capture-standalone"><span>Relevant activity</span><label><input type="checkbox" name="${stageInputName("MSA")}"> MSA</label><label><input type="checkbox" name="${stageInputName("DTM")}"> DTM</label></div>`; }
function quickCaptureConversationForm(mode,contacts) {
  const meeting=mode==="meeting";
  const steps=["person","place","learned","next"];
  const person=`${quickCapturePersonPicker(contacts,{allowNew:true})}<div class="quick-capture-new-person-fields" data-new-person-fields>${quickCaptureAdvanced(`${field("Phone number",'<input name="phoneNumber" type="tel" autocomplete="tel" placeholder="Optional">')}${field("Role",'<select name="role" data-capture-role><option>Prospect</option><option>Customer</option><option>Team</option></select>')}<div data-capture-fit>${field("Interest",`<select name="interestLevel">${INTERESTS.map(level=>`<option ${level==="Unsure"?"selected":""}>${level}</option>`).join("")}</select>`)}</div><div data-capture-fit>${field("Judgment",'<select name="judgement"><option>Good Fit</option><option>Not Good Fit</option></select>')}</div>${field("Conversation type",`<select name="conversationType">${CONVERSATION_TYPES.map(type=>`<option>${type}</option>`).join("")}</select>`)}`,"Add person details")}</div>`;
  ...
  return `<form id="quickConversationForm" class="quick-create-form quick-capture-composer quick-capture-wizard" data-capture-kind="${mode}" data-capture-steps="${steps.join(",")}" data-capture-step-index="0">${quickCaptureErrorState()}<div class="quick-capture-wizard__progress-row"><button class="quick-capture-wizard__back" type="button" data-capture-step-back aria-label="Back">${icons.chevronLeft}</button><div class="quick-capture-wizard__progress" aria-label="Capture progress"><span class="is-current"></span>${steps.slice(1).map(()=>"<span></span>").join("")}</div><p class="quick-capture-wizard__status"><span data-capture-step-number>1</span>/${steps.length}</p></div>${quickCaptureWizardStep("person",meeting?"Who was in the meeting?":"Who was it?",person,{first:true})}${quickCaptureWizardStep("place","Where?",quickCapturePlacePicker())}${quickCaptureWizardStep("learned",meeting?"What did you discuss?":"What did you learn?",learned)}${quickCaptureWizardStep("next","What's next?",next,{last:true,saveLabel:meeting?"Save meeting":"Save conversation"})}</form>`;
}
function quickCaptureCommunicationForm(mode,contacts) {
  ...
  return `<form id="quickCommunicationForm" class="quick-create-form quick-capture-composer" data-capture-kind="${mode}">${quickCaptureBack()}${quickCaptureErrorState()}<div class="quick-capture-required">${quickCapturePersonPicker(contacts)}${field("Date and time",`<input name="conversationDate" type="datetime-local" value="${dateTimeLocalValue(new Date())}" required>`)}${field(call?"What did you talk about?":"What did you discuss?",`<textarea name="notes" placeholder="Optional ${mode} notes"></textarea>`)}</div>${quickCaptureAdvanced(`${field("Direction",`<select name="direction">${COMMUNICATION_DIRECTIONS.map(direction=>`<option>${direction}</option>`).join("")}</select>`)}${field("Outcome",`<select name="outcome">${outcomes.map(outcome=>`<option>${outcome}</option>`).join("")}</select>`)}${call?field("Duration (minutes)",'<input name="durationMinutes" type="number" min="0" step="1" inputmode="numeric">'):""}${field("Follow-up",'<input name="followUpDate" type="datetime-local">')}${field("Follow-up reason",'<input name="followUpNote" placeholder="What is the next step?">')}${field("Move to stage",`<select name="pipelineStage" data-capture-stage>${quickCaptureStageOptions()}</select>`)}${field("Standalone activity",'<select name="standaloneActivity"><option value="">No change</option><option>MSA</option><option>DTM</option></select>')}`,`Add ${mode} details`)}<button class="button primary quick-capture-save" type="submit">${icons.check}<span>Log ${label.toLowerCase()}</span></button><p class="quick-capture-note">${label} activity does not increase the Conversations metric.</p></form>`;
}
function quickCaptureContactForm() { return `<form id="quickContactForm" class="quick-create-form quick-capture-composer">${quickCaptureBack()}${quickCaptureErrorState()}<div class="quick-capture-required">${field("Full name",'<input name="fullName" required autocomplete="name" placeholder="Full name">')}</div>${quickCaptureAdvanced(`${quickCapturePersonDetails({includeConversation:false})}${quickCaptureTrackingFields()}`)}<button class="button primary quick-capture-save" type="submit">${icons.userPlus}<span>Add person</span></button></form>`; }
function quickCaptureActionForm(contacts) { return contacts.length?`<form id="quickActionForm" class="quick-create-form quick-capture-composer">${quickCaptureBack()}${quickCaptureErrorState()}<div class="quick-capture-required">${quickCapturePersonPicker(contacts)}${field("Date and time",`<input name="dueDate" type="datetime-local" value="${dateTimeLocalValue(addDays(new Date(),state.settings.defaultFollowUpDays))}" required>`)}${field("Reason to reconnect",'<textarea name="note" required placeholder="What is the next step?">Follow up</textarea>')}</div><button class="button primary quick-capture-save" type="submit">${icons.calendarCheck}<span>Schedule follow-up</span></button></form>`:`${quickCaptureBack()}${EmptyState("No people yet","Add a person before scheduling a follow-up.")}`; }
function quickCaptureNoteForm(contacts) { return contacts.length?`<form id="quickNoteForm" class="quick-create-form quick-capture-composer">${quickCaptureBack()}${quickCaptureErrorState()}<div class="quick-capture-required">${quickCapturePersonPicker(contacts)}${field("Note",'<textarea name="notes" required placeholder="What is worth remembering?"></textarea>')}${field("Date",`<input name="conversationDate" type="date" max="${todayInput()}" value="${todayInput()}" required>`)}</div>${quickCaptureAdvanced(quickCaptureTrackingFields(),"Add activity details")}<button class="button primary quick-capture-save" type="submit">${icons.note}<span>Save activity</span></button></form>`:`${quickCaptureBack()}${EmptyState("No people yet","Add a person before saving an activity.")}`; }
...
function syncQuickCaptureFields(form,{hydrateRelationship=false}={}) {
  ...
  $$('[data-new-person-fields]',form).forEach(section=>{section.hidden=Boolean(selected);});
  $$('[data-new-person-name]',form).forEach(section=>{section.hidden=Boolean(selected);});
...
function syncQuickCapturePickerState(form) {
  ...
  $$('[data-capture-person-id]',form).forEach(button=>{const selected=String(button.dataset.capturePersonId)===contactId;button.classList.toggle('is-selected',selected);const marker=$('i',button);if(marker)marker.textContent=selected?'Selected':'';});
...
function syncQuickCaptureStepAction(form) {
  ...
  const key=keys[index];button.disabled=key==='person'?!String(form.elements.contactId?.value||'')&&!String(form.elements.fullName?.value||'').trim():key==='learned'?!String(form.elements.notes?.value||'').trim():false;
}
...
function renderPeopleSearchScreen() {
  ...
  const filters=peopleFiltersTrigger({className:"people-search-screen__filters"});
  const body=hasQuery
    ? `${search}<div class="people-search-screen__result-head"><p class="people-home__count">${contacts.length} ${contacts.length===1?"result":"results"}</p>${filters}</div>${renderPeopleList(contacts,activeContacts.length,{query:ui.search,emptyTitle:"No results",emptyMessage:"Try another name, place, or relationship detail.",noResults:true})}`
    : `${search}<div class="people-search-screen__result-head"><p class="people-home__count">Suggestions</p>${filters}</div>${renderPeopleSearchSuggestions(peopleVisibleContacts(getFilteredContacts({ignoreSearch:true})))}`;
  return PresentationScreen(body,{title:"Search people",eyebrow:"People",className:"people-search-screen"});
}
...
function peopleSearchText(contact) { return [contact.fullName,contact.phoneNumber,contact.placeName,contact.personalInfo,...(contact.conversations||[]).map(log=>log.notes),...(contact.notes||[]).map(note=>typeof note==="string"?note:note?.text||note?.notes||"")].filter(Boolean).join(" "); }
...
function renderPeopleSearchSuggestions(contacts) { const recent=[...contacts].sort((left,right)=>new Date(peopleActivityAt(right)||0)-new Date(peopleActivityAt(left)||0)).slice(0,4); const places=state.places.map(place=>{const related=contacts.filter(contact=>placeMatchesContact(place,contact));const latest=related.map(peopleActivityAt).filter(Boolean).sort((left,right)=>new Date(right)-new Date(left))[0]||null;return {...place,related,latest};}).filter(place=>place.related.length).sort((left,right)=>new Date(right.latest||0)-new Date(left.latest||0)).slice(0,3); if(!recent.length)return EmptyState("No recent relationships", "People you add or interact with will appear here.",{className:"people-home__empty"}); return `<section class="people-suggestions" aria-label="Search suggestions"><div class="people-suggestions__section"><h2>Recent people</h2><div class="people-suggestions__list">${recent.map(contact=>`<button type="button" class="people-suggestion-row" data-contact-id="${escapeHTML(contact.id)}">${Avatar(contact.fullName,{size:"small"})}<span><strong>${escapeHTML(contact.fullName||"Unnamed contact")}</strong><small>${escapeHTML(peopleSuggestionLabel(contact))}</small></span>${icons.chevronRight}</button>`).join("")}</div></div>${places.length?`<div class="people-suggestions__section"><h2>Recent places</h2><div class="people-suggestions__list">${places.map(place=>`<button type="button" class="people-suggestion-row people-suggestion-row--place" data-place-detail-id="${escapeHTML(place.id)}"><span class="people-suggestion-row__icon" aria-hidden="true">${icons.location}</span><span><strong>${escapeHTML(place.name)}</strong><small>${place.related.length} ${place.related.length===1?"person":"people"}${place.latest?` · ${escapeHTML(peopleRelativeDate(place.latest))}`:""}</small></span>${icons.chevronRight}</button>`).join("")}</div></div>`:""}</section>`; }
...
function renderFollowUps() {
  ...
  const primaryControls=SegmentedControl([
  ...
  ],{label:"Follow-up timing",className:"followups-home__status"});
  ...
  return `<section class="followups-home" aria-label="Follow-Ups"><header class="followups-home__header"><div class="followups-home__title"><div><span class="ui-eyebrow">Follow-through</span><h1>Follow-ups</h1></div><button type="button" class="followups-history-toggle ${selectedKind==="completed"?"is-active":""}" data-action-view="completed">Completed <span>${completedItems.length}</span></button></div><p>${selectedKind==="completed"?"A record of the promises you kept.":openItems.length?`${openItems.length} open follow-up${openItems.length===1?"":"s"} across your relationships.`:"Nothing is waiting on you right now."}</p>${primaryControls}</header>${offlineNote}${selectedKind==="completed"?`<div class="followups-home__queue followups-home__queue--completed">${followUpQueueSection("Completed",completedItems,{kind:"completed",description:"Completed follow-ups remain in relationship history.",completed:true})}</div>`:`<div class="followups-home__queue" aria-label="${escapeHTML(selectedQueue[0])} follow-ups">${followUpQueueSection(selectedQueue[0],selectedQueue[1],{kind:selectedKind,description:selectedQueue[2]})}</div>`}</section>`;
}
function followUpsLoading(){return `<section class="followups-home" aria-label="Follow-Ups" aria-busy="true"><header class="followups-home__header"><span class="ui-eyebrow">Follow-through</span><h1>Follow-ups</h1><p>Loading your relationship queue.</p></header><div class="followups-home__loading">${LoadingSkeleton({lines:4})}<span class="sr-only">Loading follow-ups.</span></div></section>`;}
function followUpEmptyState(){return `<section class="followups-empty" aria-label="No open follow-ups"><span aria-hidden="true">${icons.circleCheck}</span><div><h2>You’re caught up</h2><p>When you schedule a next step from a relationship, it will appear here.</p></div></section>`;}
function followUpQueueSection(title,items,{kind="upcoming",description="",completed=false}={}){
  const emptyCopy={overdue:"No overdue follow-ups.",today:"Nothing due today.",upcoming:"No upcoming follow-ups.",completed:"No completed follow-ups yet."}[kind]||"No follow-ups.";
  return `<section class="followup-queue followup-queue--${escapeHTML(kind)}" aria-labelledby="followup-${escapeHTML(kind)}-title"><header class="followup-queue__head"><div><span class="ui-eyebrow">${escapeHTML(kind==="completed"?"History":kind==="today"?"Today":"Follow-through")}</span><h2 id="followup-${escapeHTML(kind)}-title">${escapeHTML(title)}</h2>${description?`<p>${escapeHTML(description)}</p>`:""}</div><span class="followup-queue__count" aria-label="${items.length} ${escapeHTML(title).toLowerCase()} follow-ups">${items.length}</span></header>${items.length?`<div class="followup-queue__list">${items.map(item=>actionRow(item,{completed})).join("")}</div>`:`<p class="followup-queue__empty">${escapeHTML(emptyCopy)}</p>`}</section>`;
}
...
function followUpLastInteractionLabel(contact){const latest=latestConversationTime(contact);return latest?`Last interaction ${peopleRelativeDate(latest)}`:"No interaction recorded";}
function actionRow(item){
  const completed=followUpStatus(item)==="completed";
  const editing=ui.actionEditId===`${item.contact.id}:${item.id}`;
...
function analyticsDetailActivity(model) {
  const max=Math.max(1,...model.daySeries.map(point=>point.value));
  return `<section class="analytics-detail-section analytics-detail-activity"><h3>Activity</h3>${model.conversations.length?`<div class="analytics-detail-chart" style="--insights-points:${model.daySeries.length}" role="img" aria-label="Conversation activity: ${escapeHTML(model.daySeries.map(point=>`${point.date} ${point.value}`).join(", "))}">${model.daySeries.map(point=>`<div><span>${point.value||""}</span><i class="${point.value?"has-value":""}" style="height:${point.value?Math.max(6,Math.round(point.value/max*70)):3}px"></i><small>${escapeHTML(point.label)}</small></div>`).join("")}</div>`:emptyInline("No conversations in this period","Counted conversations will fill this chart without inventing activity.")}</section>`;
}
...
function profileDetails(c) {
  ...
  const details=[
    ["Contact",c.phoneNumber||"Not provided",`<p>${c.phoneNumber?`<a href="${phoneHref(c.phoneNumber)}">${escapeHTML(c.phoneNumber)}</a>`:"No phone number saved."}</p><p>Conversation type: ${escapeHTML(c.conversationType||"Not provided")}</p><p>Met ${escapeHTML(fmtDate(c.dateFirstMet,{month:"long",day:"numeric",year:"numeric"})||"date not provided")}</p>`],
...
function bindPageEvents(){
  ...
  $$('.today-reschedule-action').forEach(button=>button.addEventListener('click',()=>{ui.page="followups";ui.actionView="open";ui.actionEditId=button.dataset.actionId||null;window.scrollTo({top:0,left:0,behavior:"auto"});render();requestAnimationFrame(()=>document.querySelector(`.edit-action[data-action-id="${CSS.escape(ui.actionEditId||"")}"]`)?.focus());}));
...
function bindSettingsEvents(){
  ...
  $("#settingsForm")?.addEventListener("submit",async event=>{
    ...
    if(hasControl("dailyGoal"))next.dailyGoal=Number(f.get("dailyGoal"))||5;
    if(hasControl("weeklyGoal"))next.weeklyGoal=Number(f.get("weeklyGoal"))||25;
    if(hasControl("monthlyGoal"))next.monthlyGoal=Number(f.get("monthlyGoal"))||100;
...
function bindContactModalEvents(){
  ...
  $$('[data-profile-followup]').forEach(button=>button.addEventListener('click',()=>{const section=$('#profileNextAction');section?.scrollIntoView({behavior:'smooth',block:'start'});const editor=section?.querySelector('.profile-followup-editor');if(editor)editor.open=true;}));
  $$('[data-profile-reschedule]').forEach(button=>button.addEventListener('click',()=>{const editor=$('.profile-followup-editor');if(editor){editor.open=true;editor.scrollIntoView({behavior:'smooth',block:'center'});requestAnimationFrame(()=>editor.querySelector('input')?.focus());}}));
  ...
  $('#setFollowUpForm')?.addEventListener('submit',event=>{event.preventDefault();const due=new FormData(event.currentTarget).get('dueDate');if(!due)return;const active=c.followUps.filter(isScheduledFollowUp).sort((a,b)=>new Date(a.dueDate)-new Date(b.dueDate))[0];const dueDate=new Date(String(due)).toISOString();if(active)rescheduleFollowUp(active,dueDate);else createFollowUp(c,dueDate,'Follow up');c.updatedAt=nowISO();queueSave(active?'Follow-up rescheduled':'Follow-up set');render();});
  ...
  $('#removeFollowUp')?.addEventListener('click',()=>{if(!confirm('Remove this follow-up?'))return;const active=c.followUps.filter(isScheduledFollowUp).sort((a,b)=>new Date(a.dueDate)-new Date(b.dueDate))[0];if(active)transitionFollowUp(active,'deleted');c.updatedAt=nowISO();queueSave('Follow-up removed');render();});
