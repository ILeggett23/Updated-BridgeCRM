import { createBridgeFrontendFoundation } from "./ui-foundation.js?v=1.3.4";
...
function quickCaptureCommunicationForm(mode,contacts) {
  ...
  const next=`${field("Date and time",`<input name="conversationDate" type="datetime-local" value="${dateTimeLocalValue(new Date())}" required>`)}${field("Follow-up",'<input name="followUpDate" type="datetime-local">')}${field("Follow-up reason",'<input name="followUpNote" placeholder="What is the next step?">')}${quickCaptureAdvanced(`${field("Move to stage",`<select name="pipelineStage" data-capture-stage>${quickCaptureStageOptions()}</select>`)}${field("Standalone activity",'<select name="standaloneActivity"><option value="">No change</option><option>MSA</option><option>DTM</option></select>')}`,"Pipeline and activity details")}`;
...
function quickCaptureContactForm() {
  ...
  const person=`${field("Full name",'<input name="fullName" required autocomplete="name" placeholder="Who did you meet?">')}${field("Phone number",'<input name="phoneNumber" type="tel" autocomplete="tel" placeholder="Optional">')}`;
