(() => {
  ...
  async function commitRestoredState(userId, cleanState) {
    const database = await openDatabase();
    const storageId = userStorageId(userId);
    const mutationPrefix = `${userId}:`;
    await new Promise((resolve, reject) => {
      const transaction = database.transaction(["mutations", "states", "sync"], "readwrite");
      const cursorRequest = transaction.objectStore("mutations").openCursor();
      cursorRequest.onsuccess = () => {
        const cursor = cursorRequest.result;
        if (cursor) {
          if (String(cursor.key).startsWith(mutationPrefix)) cursor.delete();
          cursor.continue();
          return;
        }
        transaction.objectStore("states").put({ id: storageId, value: cleanState });
        transaction.objectStore("sync").put({ id: storageId, value: { cursor: 0, records: {}, conflicts: {} } });
      };
      cursorRequest.onerror = () => transaction.abort();
      transaction.oncomplete = resolve;
      transaction.onerror = () => reject(transaction.error);
      transaction.onabort = () => reject(transaction.error || cursorRequest.error);
    });
  }
  ...
  function flattenState(state) {
    ...
    records.set(recordKey("analytics", "primary"), { type: "analytics", id: "primary", payload: clone(state?.analytics || {}) });
  ...
  function applyRecords(state, records) {
    const next = clone(state || { contacts: [], places: [], settings: {}, analytics: {}, meta: { version: 1 } });
    ...
    for (const record of records || []) {
      ...
      } else if (record.type === "analytics" && !record.deletedAt && record.payload) {
        next.analytics = clone(record.payload);
  ...
  function queueState(nextState) {
    ...
    stateQueue = stateQueue.catch(() => {}).then(() => restoreInProgress ? undefined : queueStateInternal(snapshot));
    return stateQueue;
  }
  ...
  async function restoreBackup(id, password, confirmation = "RESTORE") {
    ...
    try {
      ...
      if (result?.state && session?.user?.id === restoringUserId) {
        ...
        lastStatus = { ...lastStatus, state: "synced", message: "Backup restored", pending: 0, conflicts: 0, authRequired: false };
        listeners.forEach(listener => {
  ...
  function renderAuthScreen({ mode = "login", message = "", error = "", email = "" } = {}) {
    ...
    app.innerHTML = `<main class="auth-shell hn-auth-shell">
      <section class="auth-card hn-auth-card" aria-labelledby="authTitle">
        <div class="hn-auth-brand"><img class="auth-logo" src="./bridge-icon-192.png?v=1.3.30" alt="" /><span>Bridge CRM</span></div>
