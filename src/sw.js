const CACHE = "bridge-app-v1.3.39";
const ROOT = new URL("./", self.location.href).href;
const APP_ROOT = new URL(ROOT);
const FOLLOW_UP_FALLBACK = new URL("?page=followups&notification=1", APP_ROOT).href;
importScripts(new URL("config.js?v=1.3.39", ROOT).href);
const API_BASE = String(self.BridgeConfig?.apiBase || "").replace(/\/+$/, "");
const apiURL = path => `${API_BASE}${path.startsWith("/") ? path : `/${path}`}`;
const SHELL = [ROOT, new URL("index.html", ROOT).href, new URL("config.js", ROOT).href, new URL("contact-logic.js", ROOT).href, new URL("engagement-logic.js", ROOT).href, new URL("communication-logic.js", ROOT).href, new URL("analytics-logic.js", ROOT).href, new URL("relationship-health-logic.js", ROOT).href, new URL("network-logic.js", ROOT).href, new URL("scorecard-logic.js", ROOT).href, new URL("release-logic.js", ROOT).href, new URL("account-client.js", ROOT).href, new URL("ui-foundation.js", ROOT).href, new URL("walkthrough.js", ROOT).href, new URL("tutorial-fixture.js", ROOT).href, new URL("app.js", ROOT).href, new URL("styles.css", ROOT).href, new URL("manifest.webmanifest", ROOT).href, new URL("bridge-mark-transparent.png", ROOT).href, new URL("bridge-icon-192.png", ROOT).href, new URL("bridge-icon-512.png", ROOT).href, new URL("bridge-icon-1024.png", ROOT).href, new URL("apple-touch-icon.png", ROOT).href];
...
self.addEventListener("push", event => {
  ...
  const options = {
    ...
    icon: new URL("bridge-icon-192.png?v=1.3.39", ROOT).href,
    badge: new URL("bridge-icon-192.png?v=1.3.39", ROOT).href,
...
self.addEventListener("message", event => {
  ...
  if (event.data?.type === "bridge-account-logout") {
    event.waitUntil(Promise.all([
      caches.keys().then(keys => Promise.all(keys.filter(key => key.startsWith("bridge-private-")).map(key => caches.delete(key)))),
