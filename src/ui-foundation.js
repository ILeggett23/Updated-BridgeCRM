...
export function createBridgeFrontendFoundation({ escapeHTML, initials, icons, getRouteState }) {
  ...
  function Tabs(items, { label = "Sections", className = "", idPrefix = "" } = {}) { const prefix = idPrefix || `ui-tabs-${++sharedPrimitiveId}`; return `<div class="ui-tabs${className ? ` ${className}` : ""}" role="tablist" aria-label="${escapeHTML(label)}" data-ui-tab-key="${escapeHTML(prefix)}"><span class="ui-tabs__indicator" aria-hidden="true"></span>${items.map(({ label: itemLabel, value, active = false, attributes = "" }) => `<button type="button" id="${escapeHTML(prefix)}-tab-${escapeHTML(value)}" role="tab" data-value="${escapeHTML(value)}" aria-selected="${active}" tabindex="${active ? "0" : "-1"}" ${attributes}>${escapeHTML(itemLabel)}</button>`).join("")}</div>`; }
  ...
  function MobileSheet(content, { title = "", id = "ui-sheet", className = "", labelledBy = "", label = "Sheet", closeAttributes = "", footer = "" } = {}) { const titleId = labelledBy || `${id}-title`; const accessibleName = title || label; const continuing = typeof document !== "undefined" && Boolean(document.getElementById(id)); return `<div class="ui-mobile-sheet-backdrop${continuing ? " is-continuing" : ""}" id="${escapeHTML(id)}" data-ui-dialog-backdrop data-ui-sheet-backdrop><section class="ui-mobile-sheet${footer ? " has-footer" : ""}${className ? ` ${className}` : ""}" role="dialog" aria-modal="true" data-ui-dialog data-ui-sheet ${title || labelledBy ? `aria-labelledby="${escapeHTML(titleId)}"` : `aria-label="${escapeHTML(accessibleName)}"`}><div class="ui-mobile-sheet__handle" aria-hidden="true" data-ui-sheet-handle></div><header data-ui-sheet-drag-region>${title ? `<h2 id="${escapeHTML(titleId)}">${escapeHTML(title)}</h2>` : ""}<button type="button" class="ui-mobile-sheet__close" aria-label="Close" data-ui-dialog-close ${closeAttributes}>${icons.close}</button></header><div class="ui-mobile-sheet__body" data-ui-sheet-scroll>${content}</div>${footer ? `<footer class="ui-mobile-sheet__footer">${footer}</footer>` : ""}</section></div>`; }
